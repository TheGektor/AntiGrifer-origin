    ...
    private void handleReplay(CommandSender sender, String[] args) {
        ...
        switch (args[1].toLowerCase()) {
            case "start" -> {
                ...
                replayEngine.startRecording(target.getUniqueId());
                sender.sendMessage(Component.text("✔ Начата запись действий игрока «" + target.getName() + "».", NamedTextColor.GREEN));
            }
            case "stop" -> {
                if (args.length < 3) {
                    sender.sendMessage(Component.text("✖ Укажите ник игрока.", NamedTextColor.RED));
                ...
                replayEngine.stopRecording(target.getUniqueId());
                sender.sendMessage(Component.text("✔ Запись действий игрока «" + target.getName() + "» остановлена.", NamedTextColor.YELLOW));
            }
            case "view" -> {
                if (args.length < 3) {
                    sender.sendMessage(Component.text("✖ Укажите ник игрока.", NamedTextColor.RED));
            ...
            case "export" -> {
                if (args.length < 3) {
                    sender.sendMessage(Component.text("✖ Укажите ник игрока для экспорта.", NamedTextColor.RED));
                    return;
                }
                org.bukkit.entity.Player target = sender.getServer().getPlayerExact(args[2]);
                if (target == null) {
                ...
                var actions = replayEngine.getReplay(target.getUniqueId());
                sender.sendMessage(Component.text("✔ Экспортировано " + actions.size() + " действий в логи сервера.", NamedTextColor.GREEN));
                // Simple console log for now as requested "export"
                System.out.println("REPLAY EXPORT [" + target.getName() + "]: " + actions.toString());
            }
            default -> sender.sendMessage(Component.text("✖ Неизвестная субкоманда replay. Используйте start | stop | view | export", NamedTextColor.RED));
        }
    }
